...
sub walk_down {
    my($self, $cb) = @_;
    # 0 here is initial depth
    # undef is 'post_callback' method
...
sub _walk_down {
    ...
    for my $child ($self->children) {
        local $Extlib::Tree::Abort = 0;
        if ($pre_cb) {
            $pre_cb->($child, $depth, $self);
        }

        unless ($Extlib::Tree::Abort) {
